...
def test(opt):
    ...
    with tqdm(desc=f'testing', total=len(test_loader.dataset), unit='img') as pbar:
        with torch.no_grad():
            for data in test_loader:
                ...
                F_model.test()
                # print("{}: phaseZ 3 {}".format(data['name'], data['phaseZ'][0,0,3].cpu().numpy()))
                ...
                pure = torch.cat([torch.cat([nearest_itpl(F_model.lr, F_model.sr.shape[-2:]),
                                  ...
                                  torch.cat([normalization(nearest_itpl(F_model.lr, F_model.sr.shape[-2:])),
    ...
    if save_mat:
        savemat(os.path.join(save_dir, 'results.mat'), {'UNetBased_pred_kernels': pred_kernels,
                                                        ...
                                                        'UNetBased_kernel_ssims': kernel_ssims,
                                                        'UNetBased_kernel_code_psnrs': kernel_code_psnrs,
                                                        'UNetBased_sr_psnrs': sr_psnrs,
                                                        'UNetBased_sr_ssims': sr_ssims,
                                                        'names': names})
