    ...
    with tqdm(desc=f'testing', total=len(os.listdir(lr_path)), unit='img') as pbar:
        with torch.no_grad():
            for name in os.listdir(lr_path):
                ...
                img = transforms.ToTensor()(Image.open(os.path.join(lr_path, name))).float().unsqueeze(0) / 65535.0 * 2.5
                # if torch.max(img) > 0.4:
                #     img = normalization(img, v_min=0.0, v_max=0.4)
                print("{}: norm 0 - {}".format(name, str(torch.max(img).numpy())))
                ...
                for pos in positions:
                    patch = img_norm[..., pos[0]:pos[0] + U_input, pos[1]:pos[1] + U_input]
                    U_model.feed_data({'lr': patch})
                    U_model.test()
                ...
                pred_kernel = torch.from_numpy(np.mean(np.array([p.detach().cpu().squeeze().numpy() for p in pred_kernel])
                                                       , axis=0))
                pred_kernels.append(normalization(pred_kernel))
                # pred_kernel = concat(pred_kernel, int(len(positions) ** 0.5), int(len(positions) ** 0.5))

                # auto normal
                # test_patch = torch.normal(mean=1, std=0.125, size=(1,1,264,264))
                # test_patch = normalization(torch.clamp(test_patch, min=0), v_min=0.0, v_max=1.0)
                # test_patch = torch.nn.functional.conv2d(test_patch, pred_kernel.unsqueeze(0).unsqueeze(0))
                # print("{}: norm 0 - {}".format(name, str(torch.max(test_patch).numpy())))
                # img = normalization(img, 0.0, torch.max(test_patch)/2)
                ...
                for pos in positions:
                    ...
                    F_model.test()
                    sr[..., pos[0] * F_scale:(pos[0] + U_input) * F_scale,
                    pos[1] * F_scale:(pos[1] + U_input) * F_scale] += F_model.sr.cpu() * mask
                    rt[..., pos[0] * F_scale:(pos[0] + U_input) * F_scale,
                    pos[1] * F_scale:(pos[1] + U_input) * F_scale] += mask
                sr /= rt
