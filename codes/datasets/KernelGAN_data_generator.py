...
from utils.KernelGAN_util import read_image, create_gradient_map, im2tensor, create_probability_map, nn_interpolation
...
class DataGenerator(Dataset):
    ...
    def __init__(self, opt, gan):
        # Default shapes
        self.g_input_shape = opt['input_crop_size']
        ...
        self.crop_indices_for_g, self.crop_indices_for_d = self.make_list_of_crop_indices(opt=opt)
    ...
    def make_list_of_crop_indices(self, opt):
        iterations = opt['max_iters']
        prob_map_big, prob_map_sml = self.create_prob_maps(scale_factor=opt['scale_factor'])
